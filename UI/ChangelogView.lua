...
GBL.CHANGELOG_DATA = {
    ...
    {"0.30.6", "2026-05-05", {
        Added = {
            "Dev-build sync isolation. A new DEV_BUILD constant in Core.lua, when set to a string on a dev branch, flips the wire version to X.Y.Z-dev.<id> so an isolated dev install cannot exchange records with production peers. The existing exact-match rejection at Sync.lua HandleHello refuses sync in both directions. The audit entry uses 'sync isolated' wording for dev-vs-release mismatches and the existing 'version mismatch' wording for real version skew. The UI surfaces the dev state via a [DEV] suffix in the main window title, a one-line login chat notification, an orange 'Dev build (vX.Y.Z) -- sync isolated' banner at the top of the Sync tab when local is dev, and a per-row '(sync isolated: this is a dev build / peer is a dev build)' tag in the Online Peers list so each unreachable peer shows the cause inline. A CI workflow step rejects any PR where DEV_BUILD is non-nil; the local test runner is intentionally unguarded so dev iteration keeps working.",
        },
        Changed = {
