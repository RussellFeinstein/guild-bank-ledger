...
- SortExecutor: fire-and-forget execution on a one-second cadence with cursor-leak safety and a source-lift guard, a stall watchdog, and a full rescan and replan at the end of every pass until the plan converges or stops shrinking. Correctness comes from convergence, not per-op proof
...
- **Active** (in guild use, improvements queued): sync (per-peer budgeting shipped; what remains is the capture-verified convergence demonstration), UI (sort and filter polish, the pagination controls, the window resize artifacts, and the Layout tab's refresh flicker, all of them queued behind the visual overhaul, where the native chassis and the one Table component replace the code that produces them rather than patching it), sort + layout (the Sort tab flow design pass and the open sort follow-ups on the tracker). Restock (new in v0.34.0): layout-driven targets and Auctionator buying are in guild use and being rebuilt from the design; the Store field on the Layout tab is the reserve-targets producer, shipped in v0.41.0.
...
One deliverable ships inside this window without gating the signal: the ledger export for WGA Raid Hub (#186, #187), which #188 builds ahead of the sync and data-model items because the hub's first paste is the first time this data leaves the guild's own installs. It carries its own integration gate rather than a release gate: the epoch-0 leak (#93) and the corrupted records on disk (#75) are repaired before that paste, and the export states what it left out and how much. It is a machine-readable format for one consumer, and is not the v1.7.0 Export row below.

- **Accessibility audit and keyboard-nav completion**: keyboard focus wired through every widget on every view, Tab/Shift+Tab through one shared key rule, focus rings verified to draw and advance across all tabs and modal dialogs, screen-reader audit, palette validation against the contrast targets the docs claim (#234 corrects the high-contrast values and the AAA claim first). The v0.3.0-v0.32.x infrastructure-without-wiring miss surfaced during the May 2026 doc sweep is the reason this is a release gate, not a Post-1.0 polish item. The verification pass is #238 and runs once, over the views the Visual overhaul rebuilds.
- **Data model integrity** (milestone): converge the declared schema, the record builders and the stored data. The MIN_SYNC_VERSION floor (#74) shipped in v0.37.0 carrying the milestone's two id-adjacent items, sync intake validation (#68) and the tab deposits and withdrawals never recorded (#67), because that release was the last compatibility break available cheaply. What remains lands as ordinary releases at no guild cost: repairing the corrupted records already on disk (#75), the defaults block (#71), the `peers` collision (#72), and the per-player category totals that are declared but never accumulated (#64). The AceDB write-path test net (#77) and the guard against raising the schemaVersion default (#76) both shipped on 2026-09-24. This sentence is a summary and not the order: the complete, current list for this milestone lives on the tracking issue (#188).
...
- **Restock rework** (milestone): the buy flow shipped working but never designed, thirteen steps behind five preconditions that failed into chat. The design landed first (#56, `docs/PLAN-restock-ux.md`) and the rebuild runs in gated steps: purchases are remembered until the ledger sees them deposited, a quote is shown and waits for a click, the list stays on screen in every state, and a blocked Search names its own reason. What is left before this signal is the rest of that order, on the milestone and on the cross-milestone tracker (#188).
- **Visual overhaul** (milestone): the roles, the access modes and the member view My record (`docs/PLAN-views-and-access.md`) over the native chassis, the theme tokens and one Table component (`docs/PLAN-visual-overhaul.md`). It absorbs History views rework: the Transactions, Gold Log and Consumption tabs shipped as data layers with rendering added later and were never designed as a set (#204), with pagination computed but no controls, the player and tab filters the README advertises without a widget, and a Type column that drops the color and icon the triple-encoding claim rests on. The rebuild lands in gated steps behind every functional arc on the cross-milestone tracker (#188). The one piece of it that could not wait shipped on its own in v0.41.2: members were seeing the whole ledger under the Own Transactions banner (#222).
