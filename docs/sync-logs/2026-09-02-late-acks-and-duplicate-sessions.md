...
- **The BUSY-abort summary gap (finding 2) is CLOSED: #202, v0.41.9, 2026-09-26.** The fix was `OnCombatStart`'s shape as predicted, tag then `FinishSending`, and the bidirectional-check ordering the issue asked to pin is pinned. Two things this record did not know. The inline teardown cleared eight send fields where `FinishSending` clears fourteen, so a BUSY-aborted session left six figures behind for the next send to report as its own, which means any capture taken after a BUSY abort and before v0.41.9 may carry that session's `lastChunkBytes` and per-chunk timings under the next send's heading. And the mid-prep branch had to be kept on the other side of the change, since `FinishSending` would write a full `Send complete 0/0 chunks` block for a preparation that never reached the wire. **The next capture is where `+ N busy +` becomes a reading for the first time**, so read it against this record's four BUSY aborts rather than against any earlier zero.
