...
| [2026-09-19-overflow-baseline-and-unviewed-tab-refusals.md](2026-09-19-overflow-baseline-and-unviewed-tab-refusals.md) | The #145 baseline on v0.39.14: a converged bank reads `frag=4 extra=2`, one run per cross-tab item. Four runs, bags on and off, whose eight refusals all came from a slot the run had written on a tab it was not viewing. |
| [2026-09-20-one-space-after-capture-and-stale-source-bypasses.md](2026-09-20-one-space-after-capture-and-stale-source-bypasses.md) | The #145 after capture on v0.39.17: the first sort on one overflow space converges to `frag=0 extra=0` at a one-time cost of 153 pack moves, 55 across the tab boundary. #191's ledger bypassed 34 pre-checks and rescued none. |
