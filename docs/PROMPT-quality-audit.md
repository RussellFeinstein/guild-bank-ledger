...
**What to investigate.** Pick the single highest-leverage module. If `Sync.lua` mutation surfaces three tests that pass under inversion of an `if`-condition, those tests need real assertions. Decide whether running mutation testing in CI is worth the time cost (it is usually 20-50x slower than the base suite).
