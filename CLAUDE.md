...
- **Version-stamp cadence (covers CHANGELOG and every other version artifact)**: a topic branch stamps every version artifact only when opening the PR, in a dedicated stamp commit. Stacked work on the topic branch leaves all of these untouched and lets `[Unreleased]` accumulate:
    ...
    - `Core.lua` `local VERSION = "..."` constant
...
- **Peer identity: always use `GBL:CanonicalPeerKey`** (Core.lua). For realm-qualified input it wraps `Ambiguate(name, "guild")`; for bare input it consults `guildData.playerRealms[name]` (built by `BuildRosterCache` on every `GUILD_ROSTER_UPDATE`, persistent across sessions) to re-realm before `Ambiguate("guild")`. **Empirical retail behavior of `Ambiguate("guild")` in connected-realm guilds:** strips realm for ALL guildmates regardless of which connected realm they sit on, not just same-realm. So bare and qualified arrivals of the same character converge on the bare form, which eliminates the duplicate-keying bug from the `Ambiguate(name, "none")` era. The trade-off is that two distinct guildmates sharing a first name across connected realms (e.g. an Alice on Tichondrius and a different Alice on Stormrage in the same guild) collide into one peer key — a known limitation of the WoW API in this context. The roster fallback is still load-bearing: it lets bare arrivals consult `playerRealms` and pass through `Ambiguate("guild")` consistently, so any future strip rule changes apply uniformly to both bare and qualified paths. Bare names without a `playerRealms` entry (cold cache, departed peer, non-guildmate sender) pass through unchanged — best effort. **Defensive validation:** the helper rejects realm strings containing hyphens (corruption from a long-fixed code path; retail realm names never contain hyphens) and falls back to bare passthrough; `RepairCorruptedPlayerRealms` (called from `BuildRosterCache`) trims corrupt entries to their first segment so offline-peer corruption doesn't persist forever. `GBL:StripRealm` is reserved for true bare-name use cases where same-name on either realm should share state — the only current consumer is `recentWhisperTargets` for chat-system-message suppression (see comment near its declaration in `Sync.lua`). The test mock for `Ambiguate("guild")` compares both raw and normalized realm forms so spaced-realm fixtures behave like retail; the mock does NOT replicate the connected-realm-strip behavior, so unit tests preserve cross-realm suffixes while in-game behavior collapses them. The `Sync.lua` `InitSync` knownPeers seed loop runs each persisted key through `CanonicalPeerKey` and consolidates `knownPeers` in place when a stale key collapses to a new canonical form. **Cold-roster gate:** `MigrateRecoverPeerRealms` returns 0 without bumping `schemaVersion` when `GetNumGuildMembers()` is 0 (cold roster); `GUILD_ROSTER_UPDATE` retriggers `MigrateAllGuilds` once per session via the `_migrationsRetried` flag so cold-roster short-circuits get a warm retry without waiting for the next login. Historical: v0.30.5 first patched the broken `Ambiguate(name, "none")` (identity in retail) with `StripRealm` (over-aggressive), then with `CanonicalPeerKey` wrapping `Ambiguate("guild")` (correct for receive-side qualified arrivals but blind to bare arrivals); the v0.30.5 follow-up added the `playerRealms` fallback layer for bare arrivals + the cold-roster premature-bump fix + the warm-roster migration retrigger + the corruption-repair helper.
...
- **Genuine root-cause fixes:** v0.11.x (ID normalization), v0.22.0 (BUSY + pending queue), v0.23.0 (stale-ACK + callback-timed ACK timer), v0.25.0 (epidemic gossip + MANIFEST), v0.25.3 (receive-timer reschedule), v0.27.0 (epoch-0 migration + unified audit), v0.28.2 (dynamic CTL threshold), v0.30.5 (peer-name realm canonicalization: replaced the broken `Ambiguate(name, "none")` at 16 sites with `GBL:CanonicalPeerKey` which wraps `Ambiguate(name, "guild")` for qualified input and consults `playerRealms` for bare input; in-game behavior of `Ambiguate("guild")` collapses guildmates of any connected realm to bare form, which eliminates same-character bare-vs-qualified duplicate keys at the cost of leaving same-name distinct-character connected-realm collisions; schema-9 migration is realm-aware; schema-11 `MigrateRecoverPeerRealms` re-realms bare keys via guild roster lookup for users who ran the intermediate StripRealm-everywhere build; v0.30.5 follow-up closed the connected-realm runtime gap discovered in smoke test by adding the bare-name `playerRealms` fallback in `CanonicalPeerKey`, fixing the schema-11 cold-roster premature-bump (return 0 without bumping when `numMembers == 0`), retriggering `MigrateAllGuilds` once on first warm `GUILD_ROSTER_UPDATE` via `_migrationsRetried`, re-canonicalizing the `InitSync` knownPeers seed loop to self-heal stuck-at-11 saved variables on next reload, and adding `RepairCorruptedPlayerRealms` to fix hyphen-corrupted realm strings carried forward from a long-fixed code path that BuildRosterCache cannot overwrite for offline peers).
...
Current: 0.30.5 (see `VERSION` file)
