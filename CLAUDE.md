...
- **src/SortExecutor.lua** — Executes a plan one op at a time with throttling (0.3s gap), pre-step verification, cursor-leak safety, replan-on-foreign-activity (cap 5), bank-close abort, and `GUILDBANKBAGSLOTS_CHANGED`-driven confirmation with timeout fallback. Confirmation is `opSucceeded` (v0.32.8): a move/split into a slot that was EMPTY or held a DIFFERENT item pre-op confirms on the destination holding the expected item with the cursor empty (the deposit is server-driven; the source-stack decrement lags past the window), while a MERGE into a slot already holding the same item still requires the src-drained predicate (move: src empty or different item; split: src.count down by at least op.count) because there an optimistic no-op pickup/drop looks identical to a real merge. An op advances via the fastest of a synchronous post-Pickup check, an interim-poll cascade (0.25/0.5/1/2s), the async event, or the 4s late-poll backstop. WoW pushes slot updates only for the currently-viewed tab, so for a destination tab the user is not viewing the executor re-queries it (`QueryGuildBankTab` at 0.5/1.5/3s while the op is in flight, gated on the dst differing from `GetCurrentGuildBankTab()`), so the deposit confirms in-window instead of one op cycle (~5s) later, without changing the user's viewed tab. When confirmation fails, the executor audits a "no-op suspected" line and falls through to the timeout-poll path. Replan-on-foreign-activity fires only when a slots-changed event with no in-flight op shows the last completed op's slots diverging from their projected post-state; a refresh echo that matches (including the executor's own re-query) is not treated as foreign activity. Before the first op, a pre-warm phase (v0.32.5) loads every unique item link in the plan via `Item:CreateFromItemLink:ContinueOnItemLoad` (3s cap) so a TWW crafted-quality client crash can't fire mid-sort; while it runs, `state.preWarming` suppresses the slots-changed handler so foreign bank activity cannot replan or step onto a cold cache (v0.32.6). `step()` refuses to issue a new op while one is already in flight (`state.waiting` set). Bank-close abort is driven by Core, not a self-registered event: the executor does NOT register `PLAYER_INTERACTION_MANAGER_FRAME_HIDE` (that would shadow Core's handler, since AceEvent keeps one callback per object+event), so `GBL:OnBankClosed` calls `GBL:_SortExecutorOnBankClosed` to abort a running sort.
