...
- **Peer identity: always use `GBL:CanonicalPeerKey`** (Core.lua). For realm-qualified input it wraps `Ambiguate(name, "guild")` (strips realm only when same as local); for bare input it consults `guildData.playerRealms[name]` (built by `BuildRosterCache` on every `GUILD_ROSTER_UPDATE`, persistent across sessions) to re-realm before `Ambiguate("guild")`. The roster fallback is what makes the helper resilient to connected-realm guilds where the same character's messages can arrive bare and qualified across paths — without it, `Ambiguate("guild")` would key bare `Katorriwl` and qualified `Katorriwl-Stormrage` distinctly. Bare names without a `playerRealms` entry (cold cache, departed peer, non-guildmate sender) pass through unchanged — best effort. Always-strip via `GBL:StripRealm` would silently collapse two distinct same-name characters across realms into one peer key. `StripRealm` is reserved for true bare-name use cases where same-name on either realm should share state — the only current consumer is `recentWhisperTargets` for chat-system-message suppression (see comment near its declaration in `Sync.lua`). The test mock for `Ambiguate("guild")` correctly compares both raw and normalized realm forms so spaced-realm fixtures behave like retail. The `Sync.lua` `InitSync` knownPeers seed loop runs each persisted key through `CanonicalPeerKey` and consolidates `knownPeers` in place when a stale key collapses to a new canonical form. **Cold-roster gate:** `MigrateRecoverPeerRealms` returns 0 without bumping `schemaVersion` when `GetNumGuildMembers()` is 0 (cold roster); `GUILD_ROSTER_UPDATE` retriggers `MigrateAllGuilds` once per session via the `_migrationsRetried` flag so cold-roster short-circuits get a warm retry without waiting for the next login. Historical: v0.30.5 first patched the broken `Ambiguate(name, "none")` (identity in retail) with `StripRealm` (cross-realm collision risk), then with `CanonicalPeerKey` wrapping `Ambiguate("guild")` (correct for receive-side qualified arrivals but blind to bare arrivals); the connected-realm runtime gap was closed in the v0.30.5 follow-up by adding the `playerRealms` fallback layer + the cold-roster premature-bump fix + the warm-roster migration retrigger.
