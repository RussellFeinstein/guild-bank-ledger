...
3. On the Restock tab, click Search to price every item the bank is short on. Until every step above is done the button is disabled and the banner says which one is missing.
4. Buy items individually with each row's Buy button, or click Buy next to buy the first row still short; each click buys one row, because the game only starts an Auction House purchase from a click. The button shows how many rows are left.
5. The Auction House quotes the real total for that quantity, and the banner shows it: click Confirm to buy (Enter works too) or Cancel to drop it with nothing spent. Untick "Confirm at price" to skip that pause and confirm each quote automatically, as long as it passes your budget and your gold.

Restock spends real gold through WoW's commodity purchase flow. It refuses any purchase you cannot afford, and you can set a gold budget to cap everything bought from one search; Buy next skips a row the budget or your gold cannot cover and moves to the next, and raising the budget brings the skipped rows back. The banner shows what the search has spent so far, counting only what Restock itself bought, so gold spent elsewhere while the window was closed changes nothing. Without Auctionator the tab still shows targets and shortfalls; only the search and buy steps need it. Every buy control is disabled until the Auction House window is open. A purchase the Auction House does not price, or does not answer, within a few seconds is given up on with a chat line rather than left waiting; a quote left unconfirmed for a minute is dropped the same way; if a confirmed purchase gets no answer, the run stops and the line says to check your mail before buying that item again. `/gbl logs` shows every auction house event the addon saw around a purchase.
