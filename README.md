...
- **Access control** — GM configures a rank threshold for full addon access. Players below the threshold get the Member view (the default: the history tabs show that person's own rows, across every character they play on the account, and nobody else's) or Sync only (the Sync tab alone, still contributing data). The promise is about what the addon shows, not about what is on disk: guild-wide sync gives every client the whole history, and the Member view is the display rule over it. Settings sync to all guild members via the HELLO protocol
