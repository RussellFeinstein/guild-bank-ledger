...
describe("Scanner", function()
    ...
    describe("full scan", function()
        ...
        -- The info read sits behind the link test (#184). v0.39.12 hoisted
        -- it above so a probe could tally what a link-less slot reported,
        -- at one C call per empty slot; the probe is gone and the read went
        -- back. The mock answers both orders identically, so this counts
        -- the calls: nothing else in the suite can tell the two apart.
        it("reads item info only for a slot with a link", function()
            MockWoW.addTab("Tab 1", nil, true)
            MockWoW.guildBank.tabs[1].slots = {
                [5] = {
                    itemLink = Helpers.makeItemLink(100, "Only Item"),
                    texture = "icon", count = 3, quality = 1, locked = false,
                },
            }

            local real = _G.GetGuildBankItemInfo
            local calls = 0
            ...
            GBL.scanInProgress = false
            GBL.bankOpen = true
            -- One tab scans synchronously: the mock fires
            -- GUILDBANKBAGSLOTS_CHANGED from QueryGuildBankTab.
            GBL:StartFullScan()
            _G.GetGuildBankItemInfo = real

            local results = GBL:GetLastScanResults()
            assert.equals(1, results[1].itemCount)
            assert.equals(3, results[1].slots[5].count)
            assert.equals(1, calls,
                "GetGuildBankItemInfo was read for slots with no link")
        end)

        -- The scan writes its per-tab summary and nothing else to the system
        -- channel (#184 removed the second line). The shared before_each has
        -- already scanned once with no tab, so the log is cleared first.
        it("writes one system line per scan, the Scan summary", function()
            MockWoW.addTab("Tab 1", nil, true)
            MockWoW.guildBank.tabs[1].slots = {
                [1] = {
                    itemLink = Helpers.makeItemLink(100, "Only Item"),
            ...
            local entries = GBL:GetLog("system")
            assert.equals(1, #entries)
            assert.is_truthy(entries[1].message:find("^Scan: "), entries[1].message)
        end)
