...
### Added
- Dev-build sync isolation: a `DEV_BUILD` constant in `Core.lua` flips the wire version to `X.Y.Z-dev.<id>` so isolated dev installs cannot exchange records with production peers. The existing exact-match rejection at `Sync.lua` HandleHello refuses both directions; the audit entry distinguishes "sync isolated" from a real version mismatch. UI surfaces the dev state via a `[DEV]` title-bar suffix, a one-line login chat notification, an orange "Dev build (vX.Y.Z) -- sync isolated" banner at the top of the Sync tab when local is dev, and a per-row `(sync isolated: this is a dev build / peer is a dev build)` tag in the Online Peers list so the cause is obvious for each unreachable peer. CI guard rejects any PR where the constant is non-nil. The local `run_tests.sh` is intentionally unguarded so dev iteration on the dev branch keeps working.
