...
- The Restock row says what is on its way and what is in doubt separately. A row holding both read "15 bought, result unknown" over the whole figure, which put a purchase that had already succeeded in doubt along with the one that had not. Each is named on its own now, with its own age.
- An item you have taken out of your bank layout with a purchase still in the mail keeps a row, under "In the mail (not in a display tab)", so you can clear it. It had no row at all before, which meant no Clear button either, and the entry came back the moment you added the item again. With the layout emptied the tab showed only the message about having no items in it, and there was no way to reach the entry at all.
...
- A purchase completed by another addon while a Restock quote was waiting is no longer taken as Restock's own: the quote is dropped with a chat line and nothing is credited. The same goes for a price that arrives while the quote waits, and for the price of a purchase you cancelled before it was quoted. Leaving the Auction House drops a waiting quote too, since the server forgets it.
...
**Reading a capture from v0.39.12 through v0.39.14:** those carry the `Scan linkless:` line on the system channel, and its absence from a later capture means nothing.
...
- Five figures in the sort log reported something other than what their name said. The sort log is what a bank problem gets diagnosed from, so each of these could send a reader to the wrong conclusion. Nothing about sorting itself changes.
  - `rescans=` is now `flushes=`. It was counting the sort's own transaction-log flushes, not rescans competing with it: the sort stops the periodic rescan while it runs, so the number was always the op count divided by fifteen. A rescan genuinely started during a sort (by ticking Auto re-scan while one is running) is now named once and counted as `extrescans=`.
  - The sort said "resumed the periodic rescan" after closing the bank, where it had not resumed anything. It says so now only when the rescan actually restarted.
  ...
  - Deficits printed in whatever order they happened to come out of the table, in both the preview and the Sort tab, so two runs on the same bank could list them differently. They are sorted by item now.

**Reading an older capture:** a sort log written before this release still uses the old names and the old meanings. `rescans=` in one of those is a flush count, and `abort=` is a stack count.
...
- If the planner ever reaches its internal limit on how many stacks it will shuffle aside to break a deadlock, the moves it had not worked out yet are now listed with the rest of the unplaced items instead of disappearing from the plan, and the stacks involved are left where they are rather than being moved elsewhere by a later step. No sort has been seen to reach that limit. The change is so that one which did would say so.

## [0.39.1] - 2026-09-08

### Fixed
- The sort no longer splits every full stack of an item to chase one odd remainder. When a stack of, say, 20 met a partial stack in overflow, it was split to fill the partial and its own remainder became the next partial, so every later stack of that item was split too: six stacks of Phoenix Oil cost eleven moves in a captured run. Full stacks now go into overflow whole and ahead of the odd stack from the same place (the bank, or one bag), and only the last full stack of an item tops up a partial, which puts the remainder at the end of the run in a single split. The same six stacks now cost six moves. When no overflow tab has a free slot at all, a full stack still tops up whatever partial has room, as before, so nothing that used to fit is left behind.
