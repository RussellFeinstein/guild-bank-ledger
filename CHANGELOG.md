...
- Sort now views the destination tab during execution when it changes, so deposits into it are visible and confirm right away. WoW refreshes slot data only for the currently-viewed guild bank tab, so a deposit into a tab you were not viewing still happened on the server but read as empty, surfacing as a false refusal and a 4-second stall on every op into that tab (clicking onto the destination tab visibly sped the sort up). The executor now selects the destination tab itself; its own resulting slots-changed event is recognized and not mistaken for another player's activity.
