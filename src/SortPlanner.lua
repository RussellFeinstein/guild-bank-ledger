...
-- What each reason code says to a player, in words. Both surfaces that
-- render plan.unplaced read this one table: SummarizeSortPlan below, which
-- /gbl sortpreview and the sort log print, and the Sort tab Unplaced list.
-- One table rather than a copy each, because #137 shipped with the preview
-- and the tab formatting the same field separately and disagreeing about
-- the same plan.
local REASON_TEXT = {
    [REASON_OVERFLOW_FULL]       = "every usable overflow tab is full",
    [REASON_NO_OVERFLOW_DEFINED] = "the layout declares no overflow tab",
    [REASON_OVERFLOW_UNVIEWABLE] = "the overflow tab is not visible to this character",
...
-- added later (#150) must show something the player can search for, and a
-- blank row is worse than a raw code. A missing reason says so plainly.
